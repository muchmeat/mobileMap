    ...
    if(locals.value && locals.value.length && locals.items){
        ...
        for(let i=0;i<locals.value.length;i++){
            ...
            if(locals.mode==="filePicker") {
                ...
                rows.push(<View key={guid()} style={[formStyle.list.fileRow,i===0?formStyle.list.fileBottomLine:null]}>
                    <View style={{flex: 1}}>
                            ...
                            if(item.id){
                                ...
                                if("AVI,WMV,MPEG,MP4,MKV,FLV,RMVB".indexOf(item.type.toUpperCase()) !== -1){
                                    if(locals.navigation)
                                        locals.navigation.dispatch(NavigationActions.navigate({routeName: "VideoPlay",params:{
                                                poster:null,
                                                uri:item.path,
                                            }
                                        }));
...
function list(locals) {
  ...
  let errorBlockStyle = formStyle.errorBlock;
    ...
    if(locals.value && locals.value.length && locals.value.length === locals.limit){
        ...
        limit =  <Text style={{color: themeStyle.form.HELP_COLOR,paddingLeft:5}}>{" (" + locals.value.length +"/"+locals.limit+")"}</Text>;
    }
